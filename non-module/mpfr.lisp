...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (import '(mpfr:mpfr-float))
  (defmethod make-load-form ((object mpfr:mpfr-float) &optional environment)
    (declare (ignore environment))
    `(let ((in (make-string-input-stream ,(write-to-string object :readably nil))))
       (mpfr::mpfr-reader in nil nil))))

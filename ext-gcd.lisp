...
(defun mod-inverse-matrix! (matrix divisor)
    ...
    (let ((result (make-array (list n n) :element-type (array-element-type matrix))))
      ...
      (dotimes (target n)
        (let ((pivot-row (do ((i target (+ 1 i)))
          ...
          (let ((inv (mod-inverse (aref matrix target target) divisor)))
            ;; process the pivot row
            (dotimes (j n)
              (setf (aref matrix target j)
                    (mod  (* inv (aref matrix target j)) divisor))
              (setf (aref result target j)
                    (mod  (* inv (aref result target j)) divisor)))
              ...
              (unless (or (= i target) (zerop (aref matrix i target)))
                (let ((factor (aref matrix i target)))
                  (loop for j from 0 below n
                        do (setf (aref matrix i j)
                                 (mod (- (aref matrix i j)
                                         (mod (* (aref matrix target j) factor) divisor))
                                      divisor))
                           (setf (aref result i j)
                                 (mod (- (aref result i j)
                                         (mod (* (aref result target j) factor) divisor))
                                      divisor)))))))))
      result)))
...
value."
  (destructuring-bind (m n) (array-dimensions matrix)
    ...
    (let ((extended (make-array (list m (+ n 1)) :element-type (array-element-type matrix))))
      ...
      (let ((rank (nth-value 1 (mod-echelon! extended divisor t))))

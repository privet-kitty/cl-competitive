...
(declaim (ftype (function * (values (or null ntt-vector) &optional)) poly-sqrt %poly-sqrt))
;; (defun %poly-sqrt (poly result-length)
;;   (declare (optimize (speed 3))
;;            (ntt-vector poly)
...
(defun %poly-sqrt (poly result-length)
  ...
  (let ((sqrt (mod-sqrt (aref poly 0) +mod+)))
    (when sqrt
      ...
      (let ((res (make-array result-length :element-type 'ntt-int :initial-element 0))
            (tt (make-array result-length :element-type 'ntt-int :initial-element 0))
            (dp (make-array 1 :element-type 'ntt-int :initial-element sqrt)))
        ...
        (loop
          ...
          do (dotimes (i len)
             ...
             (let* ((next-len (ash len 1))
                    (tmp (make-array next-len :element-type 'ntt-int :initial-element 0))
                    (tmp2 (make-array next-len :element-type 'ntt-int :initial-element 0)))
               (declare ((mod #.array-dimension-limit) next-len))

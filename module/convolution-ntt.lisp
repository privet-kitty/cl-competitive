...
(declaim (inline ensure-mint-vector))
(defun ensure-mint-vector (vector mod)
...
(defun convolve (vector1 vector2)
  ...
  (let ((n (length vector1))
    ...
    (let* ((vector1 (coerce vector1 'convolution-vector))
           ...
           (c1 (convolve1 (ensure-mint-vector vector1 +mod1+)
                          (ensure-mint-vector vector2 +mod1+)))
           (c2 (convolve2 (ensure-mint-vector vector1 +mod2+)
                          (ensure-mint-vector vector2 +mod2+)))
           (c3 (convolve3 (ensure-mint-vector vector1 +mod3+)
                          (ensure-mint-vector vector2 +mod3+)))
...
overflow."
  ...
  (let* ((mult (convolve vector1 vector2))
         (result (make-array (length mult) :element-type 'mint)))

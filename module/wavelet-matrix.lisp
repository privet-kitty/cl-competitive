...
(defun make-wavelet-matrix (bit-depth vector)
  ...
  (let* ((len (length vector))
    ...
    (declare ((mod #.array-dimension-limit) len fitted-len)
             (vector tmp))
    (loop for d from (- bit-depth 1) downto 0
          do (let ((lpos 0)
                   (rpos 0))
               (declare ((mod #.array-dimension-limit) lpos rpos))
               (dotimes (i len)
                 (let ((bit (ldb (byte 1 d) (aref tmp i))))
...
by [START, END). Returns 2^<bit depth>-1 if K is equal to END - START."
  (declare (optimize (speed 3))
           ((mod #.array-dimension-limit) k start)
  ...
  (let ((depth (wavelet-depth wmatrix))
    ...
    (loop for d from (- depth 1) downto 0
          ...
          for zero-count of-type (mod #.array-dimension-limit)
...
[START, END)."
  ...
  (let ((depth (wavelet-depth wmatrix))
    ...
    (loop for d from (- depth 1) downto 0
          for lcount = (cbv-rank (aref data d) start)
          for rcount = (cbv-rank (aref data d) end)
          for one-count of-type (mod #.array-dimension-limit) = (- rcount lcount)
          do (if (> one-count k)
                 (setq start (+ lcount (aref zeros d))
                       end (+ rcount (aref zeros d))
                       result (logior result (the wavelet-integer (ash 1 d))))

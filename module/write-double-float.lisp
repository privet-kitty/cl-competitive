  ...
  (multiple-value-bind (e string)
    ...
    (if (plusp e)
        (let* ((len (length string))
               ...
               (len-after-. (- (min (+ len-before-. max-digits) len) len-before-.)))
          (write-string string stream :end len-before-.)
          (dotimes (i (- e len))
            (write-char #\0 stream))
          (write-char #\. stream)
          (write-string string stream :start len-before-.
                                      :end (min (+ len-before-. max-digits) len))
          (when (and (not allow-trailing-point)
                     (zerop len-after-.))
            (write-char #\0 stream)))
        (let* ((len-0 (min (- e) max-digits))
               (len-after-0 (min (length string) (the fixnum (- max-digits len-0)))))
          (write-char #\. stream)
          (dotimes (i len-0)
            (write-char #\0 stream))

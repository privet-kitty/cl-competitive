...
(defun %pivot (row col a b c arow acol dict)
  ...
  (destructuring-bind (m n) (array-dimensions a)
    ...
    (rotatef (aref dict (+ m col)) (aref dict row))
...
(defun %restore (b c dict)
  ...
  (let* ((m (length b))
    ...
    (dotimes (i m)
      (let ((index (aref dict i)))
        (when (>= index m)
          (setf (aref res-primal (- index m)) (aref b i)))))
    (dotimes (j n)
      (let ((index (aref dict (+ m j))))
        (when (< index m)
          (setf (aref res-dual index) (- (aref c j))))))

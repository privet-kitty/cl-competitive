...
(defun %make-sample (size order &optional (minimum t) state)
  (let ((slopes (sort (loop repeat size collect (%random state)) order))
        (intercepts (loop repeat size collect (%random state)))

(defpackage :cp/test/rolling-hash31
  (:use :cl :fiveam :cp/rolling-hash31)
  (:import-from :cp/test/base #:base-suite))
(in-package :cp/test/rolling-hash31)
(in-suite base-suite)
...
(test rolling-hash31
  ...
  (let ((rhash1 (make-rhash "asddfddfd" :key (lambda (x) (+ 1 (char-code x))))))
    ...
    (is (= (position +rhash-mod+ *moduli-table*)
           (position +rhash-base+ *base-table*)))

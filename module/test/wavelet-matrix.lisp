...
(test wavelet-matrix/hand
  (declare (notinline make-wavelet-matrix))
  ;; Miti_7's example (http://miti-7.hatenablog.com/entry/2018/04/28/152259)
  (let* ((*test-dribble* nil)
         (miti7 (vector 5 4 5 5 2 1 5 6 1 3 5 0))
      ...
      (loop for r from (+ l 1) to 12
            do (dotimes (k (- r l))
                 (let* ((seq (vector 5 4 5 5 2 1 5 6 1 3 5 0))
                   ...
                   (sort subseq #'<)
                   (is (= (wavelet-kth-smallest w k l r)
                          (aref subseq k)))))))
      ...
      (loop for r from (+ l 1) to 12
            do (dotimes (k (- r l))
                 (let* ((seq (vector 5 4 5 5 2 1 5 6 1 3 5 0))
                        (subseq (displace seq l r)))
                   (sort subseq #'>)
                   (is (= (wavelet-kth-largest w k l r)
                          (aref subseq k)))))))
    ...
    (dotimes (l 12)
      (loop
        ...
        do (loop
             ...
             do (loop
                  for hi from lo to 7
                  do (is (= (wavelet-range-count w lo hi l r)

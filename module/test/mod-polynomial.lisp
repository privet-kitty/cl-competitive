...
(test poly-mult
  ...
  (is (equalp #(2 4 6) (poly-mult #(1 2 3) #(2) +mod+)))
  (is (equalp #(10006 10006 10006 3) (poly-mult #(1 2 3) #(-1 1) +mod+)))
  (is (equalp #() (poly-mult #(1 2 3) #() +mod+)))
  (is (equalp #() (poly-mult #() #() +mod+))))

(test poly-floor!
  (declare (notinline poly-floor!))
  (multiple-value-bind (quot rem) (poly-floor! #(8 10 -5 3) #(-3 2 1) +mod+)
    (is (equalp quot #(9996 3)))
    (is (equalp rem #(9982 41))))
  (let ((res #(0 0 0 0 0)))
    (multiple-value-bind (quot rem) (poly-floor! #(8 10 -5 3) #(-3 2 1) +mod+ res)
      (is (equalp quot #(9996 3 0 0 0)))
      (is (eq quot res))
      (is (equalp rem #(9982 41)))))
  ;; leading zeros in dividend
  (multiple-value-bind (quot rem) (poly-floor! #(8 10 -5 3 0 0) #(-3 2 1) +mod+)
    (is (equalp quot #(9996 3)))
    (is (equalp rem #(9982 41))))
  ;; leading zeros in divisor
  (multiple-value-bind (quot rem) (poly-floor! #(8 10 -5 3) #(-3 2 1 0) +mod+)
    (is (equalp quot #(9996 3)))
    (is (equalp rem #(9982 41))))
  ;; leading zeros in both arguments
  (multiple-value-bind (quot rem) (poly-floor! #(8 10 -5 3 0 0) #(-3 2 1 0) +mod+)
    (is (equalp quot #(9996 3)))
    (is (equalp rem #(9982 41))))
  ;; degenerative case
  (multiple-value-bind (quot rem) (poly-floor! #(8 10 -5 3 0 0) #(10 -5 3 0) +mod+)
    (is (equalp quot #(0 1)))
    (is (equalp rem #(8))))
  ;; division by zero
  (signals division-by-zero (poly-floor! #(8 10 -5 3) #(0 0 0 0) +mod+))
  (signals division-by-zero (poly-floor! #(8 10 -5 3) #() +mod+))
  (signals division-by-zero (poly-floor! #() #() +mod+))
  ;; null dividend
  (multiple-value-bind (quot rem) (poly-floor! #() #(10 -5 3 0) +mod+)
    (is (equalp quot #()))
    (is (equalp rem #()))))

(test poly-mod!
  (declare (notinline poly-mod!))
  (is (equalp #(9982 41) (poly-mod! #(8 10 -5 3) #(-3 2 1) +mod+)))
  ;; leading zeros in dividend
  (is (equalp #(9982 41)
              (poly-mod! #(8 10 -5 3 0 0) #(-3 2 1) +mod+)))
  ;; leading zeros in divisor
  (is (equalp #(9982 41)
              (poly-mod! #(8 10 -5 3) #(-3 2 1 0) +mod+)))
  ;; leading zeros in both arguments
  (is (equalp #(9982 41)
              (poly-mod! #(8 10 -5 3 0 0) #(-3 2 1 0) +mod+)))
  ;; degenerative case
  (is (equalp #(8) (poly-mod! #(8 10 -5 3 0 0) #(10 -5 3 0) +mod+)))
  ;; division by zero
  (signals division-by-zero (poly-mod! #(8 10 -5 3) #(0 0 0 0) +mod+))
  (signals division-by-zero (poly-mod! #(8 10 -5 3) #() +mod+))
  (signals division-by-zero (poly-mod! #() #() +mod+))
  ;; null dividend
  (is (equalp #() (poly-mod! #() #(10 -5 3 0) +mod+)))
  (is (equalp #() (poly-mod! #(0) #(10 -5 3 0) +mod+))))
...
(test poly-integrate
  (is (equalp #() (poly-integrate #() +mod+)))
  (is (equalp #(0 3 5004 6672 1) (poly-integrate #(3 1 2 4) +mod+)))
  (is (equalp #(0 3) (poly-integrate #(3) +mod+))))

(test poly-shift!
  (declare (notinline poly-shift!))
  (is (equalp #(6 1 2 4) (poly-shift! #(3 1 2 4) 3 +mod+)))
  (is (equalp #(3 1 2 4) (poly-shift! #(3 1 2 4) 0 +mod+)))
  (is (equalp #(6) (poly-shift! #(3) 3 +mod+)))
  (is (equalp #(3) (poly-shift! #() 3 +mod+)))
  (is (equalp #() (poly-shift! #(3) -3 +mod+)))
  (is (equalp #() (poly-shift! #() 0 +mod+)))
  (is (equalp #(3) (poly-shift! #(3) 0 +mod+))))

(test poly-scale!
  (declare (notinline poly-scale!))
  (is (equalp #(9 3 6 12) (poly-scale! #(3 1 2 4) 3 +mod+)))

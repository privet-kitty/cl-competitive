    ...
    (let ((n (length f))
      ...
      (let ((i 0))
        ...
        (loop for j from 1 below (- n 1)
              do (loop for k of-type (mod #.array-total-size-limit)
      ...
      (do* ((mh 1 m)
            (m (ash mh 1) (ash mh 1)))
           ((> m n))
        (declare ((mod #.array-total-size-limit) mh m))
        (let ((mq (ash mh -1)))
          (setq theta (/ theta 2))
          (do ((jr 0 (+ jr m)))
              ((>= jr n))
            (declare ((mod #.array-total-size-limit) jr))
              ...
              (do ((j 0 (+ j m)))
                ...
                (let* ((j+mh (+ j mh))
                       ...
                       (ximag (- (* wreal (aref f j+m-i))
                                 (* wimag (aref f (+ j+mh i))))))
                  (declare ((mod #.array-total-size-limit) j+mh j+m-i))
                  (setf (aref f (+ j+mh i))
                        (+ (- (aref f (- j+mh i))) ximag))
                  (setf (aref f j+m-i)
                        (+ (aref f (- j+mh i)) ximag))
  ...
  (prog1 f
    (let* ((n (length f))
      ...
      (do* ((m n mh)
            (mh (ash m -1) (ash m -1)))
           ((zerop mh))
        (declare ((mod #.array-total-size-limit) m mh))
        (let ((mq (ash mh -1)))
          (do ((jr 0 (+ jr m)))
              ((>= jr n))
            (declare ((mod #.array-total-size-limit) jr))
...
;; (i*theta) is efficient.
;; TODO: we can do the same for the maximum possible length instead of a fixed
;; length.
(defun %make-trifunc-table (n)
  (declare (optimize (speed 3) (safety 0)))
  (check-type n (mod #.array-total-size-limit))
  (assert (power2-p n))
...
(defun %dft-fixed-base! (f)
  ...
  (prog1 f
    (let* ((n (length f))
           ...
           (sin-table *sin-table*)
           (factor n))
      (declare ((mod #.array-total-size-limit) factor)
